...
class Model:
    ...
    def authPatient(self, data):                                        #Funktion som validerer, at CPR består af 10 tal
        try:
            int(data["cpr"])
            if (len(data["cpr"]) == 10):
                self.valid = "Yes"
...
class Root(Tk):
    def __init__(self):
        ...
        self.Timelabel.grid(row = 1, column = 0, sticky = E, padx=40)
        ...
        self.Datelabel.grid(row = 1,  column = 0, sticky = E, padx=110)
...
class EKGView(Frame):
    def __init__(self, *args, **kwargs):
        ...
        self.header = Label(self, text="EKG for patient", font=("Helvetica bold", 30))
        self.header.grid(row=0, column=0, pady=70, sticky=S, columnspan=2)
        ...
        self.HRValue = Label(self.HRFrame, text="60", font=("Sergoe UI", 30))
        self.HRValue.grid(row=0, column=1, padx=20, pady=20)
